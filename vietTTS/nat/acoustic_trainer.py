...
def loss_fn(params, aux, rng, inputs: AcousticInput, is_training=True):
    ...
    mel_hat, new_aux = model.apply(params, aux, rng, inputs)
    loss = (jnp.square(mel_hat - mels) + jnp.square(mel_hat - mels)) / 2
    loss = jnp.mean(loss, axis=-1)
    num_frames = (inputs.wav_lengths // (FLAGS.n_fft // 4))[:, None]
    mask = jnp.arange(0, L)[None, :] < num_frames
    loss = jnp.sum(loss * mask) / jnp.sum(mask)
    return (loss, new_aux) if is_training else (loss, new_aux, mel_hat, mels)
...
lr_schedule = optax.exponential_decay(FLAGS.learning_rate, 100_000, 0.5, staircase=True)
...
def initial_state(batch):
    ...
    batch = batch._replace(mels=batch.mels[:, ::2])
...
def train():
    ...
    train_data_iter = load_textgrid_wav(
        ...
        FLAGS.batch_size * num_devices * FLAGS.steps_per_update,
        FLAGS.max_wave_len,
        "train",
    )
    ...
    tr = tqdm(
        range(last_step + 1, FLAGS.num_training_steps + 1, FLAGS.steps_per_update),
        desc="training",
    ...
    def batch_reshape(batch):
        return jax.tree_map(
            lambda x: jnp.reshape(
                x, (num_devices, FLAGS.steps_per_update, -1) + x.shape[1:]
            ),
            batch,
        )
